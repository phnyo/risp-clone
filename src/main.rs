use core::fmt;
use std::{collections::HashMap, num::ParseFloatError, io, rc::Rc};
...
macro_rules! ensure_tonicity {
    ($check_fn: expr) => {{
        |args: &[RispExp]| -> Result<RispExp, RispErr> {
            ...
            let first = floats.first().ok_or(RispErr::Reason("expected a number".to_string()))?;
            let rest = &floats[1..];
...
fn eval_lambda_args(arg_forms: &[RispExp]) -> Result<RispExp, RispErr> {
    ...
    if arg_forms.len() > 2 {
        return Err(
            RispErr::Reason(
                "fn has to be defined with two values".to_string()
            ),
        )
    }
        ...
        .map(
            |x| {
                match x {
                    RispExp::Symbol(s) => Ok(s.clone()),
                    _ => Err(RispErr::Reason(
                            "expected symbols in the argument".to_string())
                    ),
                }
            }
        )
...
fn main() {
    ...
    loop {
        ...
        match parse_eval(expr, env) {
            ...
            Err(e) => match e {
                RispErr::Reason(msg) => println!("// err => {}", msg),
            },
        }
    }
}
